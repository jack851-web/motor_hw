...
// 安全获取uint32参数：逻辑与get_param_as_float对称，类型错误返回0
...
// 快速判断参数是否存在：利用unordered_map的find，无插入副作用
...
// 构造函数：遍历data_ptr_中所有电机配置，动态创建Motor实例并注册到双ID映射表
    ...
    :  data_ptr_(data_ptr) 
{
    ...
    serial_.open();
    usleep(1000000);//1s  // 等待串口驱动就绪
    RCLCPP_INFO(rclcpp::get_logger("motor_hw"), 
        "Serial port %s opened successfully with baudrate %d", serial_port.c_str(), seial_baud);
    for(auto& it : motors)
    {
        switchControlMode(*it.second,damiao::MIT_MODE);
        RCLCPP_INFO(rclcpp::get_logger("motor_hw"), 
...
{   
  ...
  //  }
    ...
    {
      rec_thread.join();  // 等待接收线程安全退出，避免野指针
    }
    if (serial_.isOpen())
    {
      serial_.close(); 
...
void Motor_Control::control_mit(Motor &DM_Motor, float kp, float kd, float q, float dq, float tau)
{
    ...
    // 8字节数据帧：q(16)+dq(12)+kp(12)+kd(12)+tau(12)，无浪费位
 ...
 */
// 双ID插入：SlaveId与MasterId映射到同一Motor指针，支持主从切换
